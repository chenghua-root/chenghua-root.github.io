...
## 概述  
进程（线程）管理、虚拟内存和文件系统是单机系统最重要的几个底层原理。本文主要讲解Linux内核进程相关的内容，包括进程管理与调度，用户态与内核态，中断服务，进程同步，时钟机制等。  
...
应用程序在用户空间执行，它们只能看到允许它们使用的部分系统资源，并且只能使用某些特定的系统功能，不能直接访问硬件，也不能访问内核划给别人的物理内存。  
内核运行时，系统以内核态进入内核空间执行。  
执行普通用户程序时，系统将以用户态进入以用户空间执行。  
